...
<div class="col-sm-12">
    <div id="frame">
            ...
            <div id="sidepanel">
                <div id="profile">
                    <div class="wrap">
                        <img id="profile-img" @if(\App\User::where("id", $userID)->first()->profil == null) src="images/profil/profil.png" @else  src="images/profil/{{\App\User::where("id", $userID)->first()->profil}}" @endif class="online" alt="" />
                        <p>{{\App\User::where("id", $userID)->first()->username}}</p>
        ...
        <div class="content">
            <div class="contact-profile">
                <img id="conseiller-profil" src="images/profil/profil.png" alt="" />
                <p id="conseiller-name">@if(count($users) > 0 ) {{$users[0]->username}} @else User @endif</p>
            ...
            <div class="messages" id="message">
                <ul>
                    @foreach($chats as $chat)
                        @if($chat->sender_id == \App\User::where("id", $userID)->first()->id)
                            <li class="replies" style="margin-top: 5px; margin-bottom: 15px;">
                            <img @if($utilisateur->profil == null) src="images/profil/profil.png" @else  src="images/profil/{{\App\User::where("id", $userID)->first()->profil}}" @endif alt="" />
            ...
            <div class="message-input">
                <div class="wrap">
                    ...
                    <input type="hidden"  value="{{$userID}}" id="sender"/>
...
<script >
    ...
    function loadDataForSpecifiqUser(id){
        ...
        $.ajax({
            type : "POST",
            url:"{{route('retrieveChatForSpecificUserAssistance')}}",
            data:{
                ...
                if(data["error"] === false){
                    ...
                    for (var i=0 ; i< data["chats"].length ; i++) {
                        if(data["chats"][i].sender_id == userID){
                            $('<li class="replies" style="margin-top: 5px; margin-bottom: 15px;"><img  @if(\App\User::where("id", $userID)->first()->profil == null) src="images/profil/profil.png" @else  src="images/profil/{{\App\User::where("id", $userID)->first()->profil}}" @endif alt="" /><p>' + data["chats"][i].message + ' <br/> <span style="font-size: 10px;float: right;">'+data["chats"][i].created_at+'</span></p></li>').appendTo($('.messages ul'));
                        }else{
                            //alert(data["chats"][i].sender_id);
                            $('<li class="sent" style="margin-top: 5px; margin-bottom: 15px;"><img  src="images/profil/profil.png" alt="" /><p>' + data["chats"][i].message + ' <br/> <span style="font-size: 10px;float: left;">'+data["chats"][i].created_at+'</span></p></li>').appendTo($('.messages ul'));
                        }
    ...
    function newMessage() {
        ...
        $('<li class="replies" style="margin-top: 5px; margin-bottom: 15px;"><img @if(\App\User::where("id", $userID)->first()->profil == null) src="images/profil/profil.png" @else  src="images/profil/{{\App\User::where("id", $userID)->first()->profil}}" @endif alt="" /><p>' + message + ' <br/> <span style="font-size: 10px;float: right;">'+output+'</span></p></li>').appendTo($('.messages ul'));
        ...
        $.ajax({
            type : "POST",
            url:"{{route('sendChatMessageAssistance')}}",
            data:{
                '_token': '{{Session::token()}}',
                'message': message,
                'receiverID': userID,
    ...
    function retrieveChat(){
        $.ajax({
            type : "POST",
            url:"{{route('retrieveChatAssistance')}}",
            data:{
                '_token': '{{Session::token()}}',
                'id': userID,
                'senderID': senderID,
    ...
    function isTyping(){
        $.ajax({
            ...
            data:{
                '_token': '{{Session::token()}}',
                'id': "{{\App\User::where("id", $userID)->first()->id}}"
            },
            success: function(data){
                notTyping();
            },
