...
class PairEducateurController extends Controller
{
    ...
    public function saveComptePE(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'numero.regex' => 'Le format du téléphone est invalide. Ex format valide : ND123456'
        ]);
...
//        $editNumber = $request["numero"];
            ...
            $excel->sheet('ExportFile', function ($sheet) use ($users, $typeUser) {
                ...
                foreach ($users as $user) {
                    ...
                    if ($typeUser != "admin regionaux") {
                        $superieur = \App\User::where("id", \App\AffectationPE::where("educateur_id", $user->id)->first()->affectant_id)->first()->paireducateur->code;
                    }
    ...
    {
        info('postSaveSuperviseurUniv request', ['request' => $request->all()]);
        ...
        try {
            ...
            info('Exception', ['exception' => $exception]);
    ...
    public function postSaveEntretienInfo(Request $request)
    {
        $this->validate($request, [
            'code.*' => [
                'bail',
                'size:8',
                'custom_regex:/^ND\d{6}$/,/^\d{8}$/',
                'custom_range:ND000001,ND060000',
                // 'regex:/^ND\d{6}$/',
            ]
        ], [
            'code.*.size' => 'Le numéro doit etre exactement huit(8) chiffres',
            'code.*.custom_regex' => 'Format du numéro invalide',
            'code.*.custom_range' => 'Le numéro doit etre compris entre ND000001 et ND060000',
        ]);
    ...
    public function getNouveauCode($code, $sexe, $age, $country, $profession, $region)
    {
        ...
        $contains_nd = str_contains($code, 'ND');
