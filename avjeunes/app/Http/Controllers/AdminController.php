...
class AdminController extends Controller
{
            ...
            $excel->sheet('ExportFile', function($sheet) use($users) {
                ...
                foreach($users as $user) {
                    $data[] = array(
                        ...
                        $dateJour->diffInYears(new \Carbon\Carbon($user->datenaissance)) == null ? 0 : $dateJour->diffInYears(new \Carbon\Carbon($user->datenaissance)) ,
    ...
    public function getListePresenceTeleConseiller(){
        ...
        if(isset($_GET["debut"]) && isset($_GET["fin"])){
            ...
            $presences = DB::select("SELECT * FROM presences where created_at BETWEEN '".$debut." 00:00:00' and  '".$fin." 23:59:59' group by nom");
            ...
            $presences = DB::select("SELECT * FROM presences group by nom");
    ...
    public function getListeOneTeleConseiller(){
        ...
        if(isset($_GET)){
            ...
            if($fin == null && $debut == null){
                ...
                $presences = DB::select("select * from presences where nom =\"".$acteur->nom."\" and created_at between '".$debut."  00:00:00' and '".$fin." 23:59:59' order by id desc");
            }
    ...
    public function getAdminHealthPage(){
        $users = DB::select("select * from users where type_user_id in (11,12) order by id desc");
    ...
    public function postSaveWebSerie(Request $request)
    {
        ...
        $serie = WebSerie::create(
            array_merge(
                $request->only('titre', 'url', 'description', 'userDash'),
                [
