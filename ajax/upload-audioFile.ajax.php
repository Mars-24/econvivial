<?php
	...
	if (is_array($_FILES)) {
		...
		foreach ($_FILES['audiofiles']['name'] as $name => $value) {
			
			$extensions_valides = array('mp3', 'wav', 'aac', 'oga');
			$extension_upload = strtolower(substr(strrchr($_FILES['audiofiles']['name'][$name], '.') ,1));
			$newName=md5(rand()).'.'.$extension_upload;
	
	
			if (($_FILES['audiofiles']['size'][$name][$value])<='6000000'){
				if (!in_array($extension_upload,$extensions_valides)){
					$data="Un (des) audio(s) semble invalide !";
				}else{
					move_uploaded_file($_FILES['audiofiles']['tmp_name'][$name], '../uploads/audios/AUD-'.$newName);

					$message = '%%$aud$%%-AUD-'.$newName;
				...
				$data="Un (des) audio(s) semble trop volumineux | 5 MB";
			}
		}
		echo $data;
	}
