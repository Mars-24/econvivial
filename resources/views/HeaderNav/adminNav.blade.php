                    ...
                    @foreach(Auth::user()->unreadNotifications as $notification)
                        <a class="dropdown-item preview-item" href="#">
                            ...
                            <div class="preview-item-content">
                                ...
                                <!-- IMPORTANT : Cannot import the class basename, not conform to the blade filename -->
                                {{-- @include('HeaderNav.Notifications.'.snake_case(class_basename($notification->type))) --}}
                                
                                @include('HeaderNav.Notifications.replied_to_thread')
        ...
        <ul class="navbar-nav navbar-nav-right">
            <li class="nav-item dropdown d-none d-lg-flex">
                <a class="nav-link dropdown-toggle" id="languageDropdown" href="#" data-toggle="dropdown">
                    {{ auth()->user()->typeuser->libelle .' : '. auth()->user()->username }}
                    <img src="{{ auth()->user()->avatar }}" alt="{{ auth()->user()->username }}" style="height: 30px; height: 30px; border-radius: 100%;margin-left: 5px;">
                </a>
