                                    ...
                                    font-weight: 400;
                                                ...
                                                {{-- @foreach($details as $detail)
                                                    <tr>
                                                        ...
                                                        <td>
															@foreach($menstrus as $menstru)
																@if($menstru->id == $detail->menstru_id)
																	{{ date_format(date_create($menstru->dateRegle),"d M Y") }}
																@endif
															@endforeach
														</td>
                                                        <td>
                                                        ...
                                                        <td><b>
															@foreach($menstrus as $menstru)
																@if($menstru->id == $detail->menstru_id)
																	@if($rapport->typeAlerte == "Regle")
																		{{date_format(date_create($menstru->dateProchainRegle),"d M Y")}}
																	@else
																		{{date_format(date_create($menstru->dateProchainOvulation),"d M Y")}}
																	@endif
																@endif
															@endforeach
                                                            </b></td>
                                                        <td>
                                                            @if(!$detail->recu)
                                                                <label style="background-color: hsl(356, 84%, 50%); padding : 2px ; border-radius:12px;" class="badge badge-danger badge-pill">NON</label>
                                                            @else
                                                                <label  style="background-color: hsl(257, 79%, 20%); padding : 2px ; border-radius:12px;" class="badge badge-success badge-pill">OUI</label>
                                                            @endif
                                                        </td>
                                                    </tr>
                                                @endforeach --}}
