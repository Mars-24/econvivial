<?php
	...
	$data='';
	$extensions_valides = array('pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'txt', 'html');

	if (is_array($_FILES)) {
		
		foreach ($_FILES['files']['name'] as $name => $value) {

			$extension_upload = strtolower(substr(strrchr($_FILES['files']['name'][$name], '.') ,1));
			$newName=md5(rand()).'.'.$extension_upload;
	
			if (($_FILES['files']['size'][$name][$value])<='‪20000000‬'){
				if (!in_array($extension_upload, $extensions_valides)){
					$data="Un (des) document(s) semble invalide !";
				}else{
					move_uploaded_file($_FILES['files']['tmp_name'][$name], '../uploads/docs/DOC-'.$newName);

					$message = '%%$doc$%%-DOC-'.$newName;
				...
				$data="Un (des) document(s) semble trop volumineux | 20 MB";
			}
		}
		echo $data;
	}
