<?php
	...
	if (is_array($_FILES)) {
		...
		foreach ($_FILES['picsfiles']['name'] as $name => $value) {
			
			$extensions_valides = array('jpg', 'jpeg', 'png', 'gif', 'webp');
			$extension_upload = strtolower(substr(strrchr($_FILES['picsfiles']['name'][$name], '.') ,1));
			$newName=md5(rand()).'.'.$extension_upload;
	
	
			if (($_FILES['picsfiles']['size'][$name][$value])<='‪20000000‬'){
				if (!in_array($extension_upload,$extensions_valides)){
					$data="Une (des) image(s) semble invalide !";
				}else{
					move_uploaded_file($_FILES['picsfiles']['tmp_name'][$name], '../uploads/img/IMG-'.$newName);

					$message = '%%$img$%%-IMG-'.$newName;
				...
				$data="Une (des) image(s) semble trop volumineuse | 5 MB";
			}
		}
		echo $data;
	}
