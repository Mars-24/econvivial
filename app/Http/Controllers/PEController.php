...
class PEController extends Controller
{
    ...
    public function generateRapport(Request $request)
    {
        ...
        $rapportUserPe->destinataire = User::where("id", AffectationPE::where("educateur_id", $userID)->first()->affectant_id)->first()->id;
        $rapportUserPe->save();
    ...
    public function importEntretienPair(Request $request)
    {
        ...
        try {
            $this->validate($request, [
                'fichierImporte' => 'required|file|max:10240|mimes:xls,xlsx',
            ]);
           ...
           Excel::filter('chunk')->noHeading()->load($url)->chunk(3000, function ($results) {
                //return dd($results);
                foreach ($results as $value) {
                    if (!empty($value)) {
                        $matches=[];
                        preg_match_all('!\d+!', $value['condom'], $matches);
                        $codification = array("L" => "LC", "GRAND LOME" => "LC", "M" => "MT", "MARITIME" => "MT", "P" => "PT", 
						"PLATEAUX" => "PT", "C" => "CT", "CENTRALE" => "CT",
                            "K" => "KR", "KARA" => "KR", "S" => "SV", "SAVANES" => "SV", "Employé" => "Employé", 
							"Elève" => "Elève", "Eleve" => "Eleve", "Etudiant" => "Etudiant", "Apprenti" => "Apprenti", 
							"Entrepreneur" => "Entrepreneur", "Employé" => "Employé", "Fonctionnaire" => "Fonctionnaire", "Autres" => "Autres");
							
                        //dd(Carbon::createFromFormat('Y-m-d H:i:s', $value["cree_le"])->format('d-m-Y'));
                        //$details = new EntretienDetails(Carbon::now(), $value["code"], $value["sexe"], $value["profession"], $value["region"], $value["age"], $value["type_activite"], $value["type_entretien"], $value["statut"], $matches[0][0], $matches[0][1], $value["referer"]);
                        $UNIX_DATE = ($value["cree_le"] - 25569) * 86400;
                        $date_column = gmdate("Y-m-d H:i:s", $UNIX_DATE);
                        try{
                            $details = new EntretienDetails($date_column, $value["code"], $value["sexe"], $value["profession"], $value["region"], $value["age"], $value["type_activite"], $value["type_entretien"], $value["statut"], $matches[0][0], $matches[0][1], $value["referer"]);
                        }catch (Exception $e){
                        ...
                        if (Auth::user() != null && !empty($details->getTypeActivite()) && !empty($details->getTypeEntretien()) && !empty($details->getRegion())) {
                            ...
                            try{
                                ...
                                $entretien->timestamps = false;
                                $entretien->created_at = $details->getDate();
                                $entretien->updated_at = date("Y-m-d H:i:s");
                                $entretien->save();
                            }catch (Exception $e){
                            ...
                            try{
                                ...
                                $regionAutres = Region::where('code', 'AUT')->first();
                                $newRegion = $region ?: $regionAutres;
                                if($newRegion == $regionAutres)
                                    dd($region);
    ...
    public function getNouveauCode($code, $sexe, $age, $country, $profession, $region)
    {
        ...
        // Récupérer les correspondances par rapport aux (4) derniers chiffres du numéro de téléphone
        $last_numb_tel = substr($code, 4, 4);
