...
class ResultatConsultationController extends DataTableController
{
    public function __invoke(Request $request)
    {
        $query = $this->query(Consultation::where('result', true))
            ->with(['formationSanitaire', 'user', 'objetConsultation', 'resultatConsultations'])
            ->range($request->range)
            ->query;
