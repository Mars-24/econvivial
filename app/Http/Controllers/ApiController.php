...
class ApiController extends Controller
{
    ...
    //Récupérer les informations du bénéficiare par numéro
    //https://econvivial.org/api/v1/econvivial/beneficiaire-info/92104584
    public function getBeneficiaireInfo($numero)
    {
        $result["beneficiaireInfo"] = [];
        $result["error"]  = false;

        $beneficiareNumero = str_replace( ' ', '', $numero);
        $beneficiareNumero = '%228'.$beneficiareNumero;
        $beneficiaire = DB::select('select * from beneficiaires where telephone like :phone',['phone' => $beneficiareNumero]);
        //dd($beneficiaire);
        $site = DB::select('select * from site_consults where id_site = :id',['id' => $beneficiaire[0]->id_site]);
        $lsite = ($site == null)? null :$site[0];
        $quartier_site = ($lsite == null)? null : DB::select('select * from quartiers where id = :id',['id' =>$site[0]->id_site]);
        $region = DB::select('select * from regions where id = :id',['id' => $beneficiaire[0]->region_id]);
        $quartier = DB::select('select * from quartiers where id = :id',['id' =>$beneficiaire[0]->quartier_id]);

        $infos = [];
        if($beneficiaire != null){
            $infos["id"] = $beneficiaire[0]->id;
            $infos["numCarnet"] = $beneficiaire[0]->num_carnet;
            $infos["nom"] = $beneficiaire[0]->nom;
            $infos["code"] = $beneficiaire[0]->code;
            $infos["prenom"] = $beneficiaire[0]->prenom;
            $infos["telephone"] = $beneficiaire[0]->telephone;
            //$infos["ptme"] = $beneficiaire->ptme;
            //$infos["haveConjoint"] = $beneficiaire->haveConjoint;
            $infos["optionSuivi"] = $beneficiaire[0]->optionSuivi;
            $infos["dateRegle"] = $beneficiaire[0]->dateRegle;
            $infos["dureeGrossese"] = $beneficiaire[0]->dureeGrossese;
            $infos["dateAccouchement"] = $beneficiaire[0]->dateAccouchement;
            $infos["ageBebe"] = $beneficiaire[0]->ageBebe;
            $infos["nomBebe"] = $beneficiaire[0]->nomBebe;
            $infos["dateFinSuivi"] = $beneficiaire[0]->dateFinSuivi;
            $infos["telephoneConjoint"] = $beneficiaire[0]->telephoneConjoint;
            $infos["site"] = ($quartier_site == null)? null: $quartier_site[0]->libelle;
            $infos["region"] = ($region == null)? null:$region[0]->libelle;
            $infos["quartier"] = ($quartier == null)? null:$quartier[0]->libelle;

            $result["beneficiaireInfo"] = $infos;
            $result["error"] = false;
    ...
    //Récupérer les informations du cpn du bénéficiaire par id
    //https://econvivial.org/api/v1/econvivial/beneficiaireCPN-info/1
    public function getBeneficiaireInfoCPN($id)
    {
        $result["message"] = [];
        $result["error"] = false;

        $cpnInfos = DB::select('select * from cpn_suivis where beneficiaire_id = :id', ['id' => $id]);

        if ($cpnInfos != null) {

            $result["dateprochaineCPN"] = $cpnInfos[0]->dateProchaineCPN;
            $result["message"] = "Informations chargées";
            $result["error"] = false;

            return response()->json($result);
        } else {
            $result["message"] = "Vous n'avez pas encore fait de Consultation";
            $result["error"] = false;
    ...
    //Récupérer les informations du cpn du bénéficiaire par id
    //https://econvivial.org/api/v1/econvivial/beneficiaireVaccination-info/1
    public function getBeneficiaireInfoVaccination($id)
    {
        $result["message"] = [];
        $result["error"]  = false;

        $benef = DB::select('select * from beneficiaires where id = :id',['id' => $id]);
        $vac = DB::select('select * from vaccin_suivis where beneficiaire_id = :id',['id' => $id]);
        if($vac != null){
            $lib_vac = DB::select('select * from vacci_types where code_vacci_typ = :type',['type' => $vac[0]->code_vacci_typ]);
            $result["dateProchaineVaccin"] = $vac[0]->dateprochain_vaccin;
            $result["vaccin"] = $lib_vac[0]->lib_vacci_typ;
            $result["dateFinSuivi"] = $vac[0]->datefin_vaccin;
            $result["nomBebe"] = $benef[0]->nomBebe;
            $result["ageBebe"] = $benef[0]->ageBebe;
            ...
            $result["message"] = "Vous n'avez pas encore de vaccination programmé";
            $result["error"] = false;
    ...
    //Récupérer les informations du cpn du bénéficiaire par id
    //https://econvivial.org/api/v1/econvivial/beneficiaireVaccination-info/1
    public function getBeneficiaireInfoPF($id)
    {
        $result["message"] = [];
        $result["error"]  = false;

        $benef = DB::select('select * from beneficiaires where id = :id',['id' => $id]);
        $pf = DB::select('select * from suivi_planningFA where beneficiaire_id = :id',['id' => $id]);
        if($pf != null){
            $lib_pf = DB::select('select * from plannings where id = :type',['type' => $pf[0]->planning_id]);
            $result["dateContraception"] = $pf[0]->dateContraception;
            $result["dureeContraception"] = $pf[0]->dureeContraception;
            $result["titre"] = $lib_pf[0]->titre;
