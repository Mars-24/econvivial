...
class AssistanceLigneController extends Controller
{
    ...
    public function postPresenceArriver(Request $request){
		...
		$tele = DB::table('teleconseillers')
			->where('teleconseillers.nom', '=', $request->nom)
			->where('teleconseillers.prenom', 'like', '%'.$request->prenom.'%')
			->where('teleconseillers.etat', '=', 1)
			->first();
        ...
        $presence->programmer = $request["programmer"];
        $presence->periode = $request["periode"];
        $presence->teleconseiller_id = $tele->id;
        $presence->user_id = Auth::user()->id;
		...
		$verification = DB::table('presences')
			->where("teleconseiller_id", "=", $tele->id)
		...
		if($verification<=1)
		{
			...
			return redirect()
			->back()
			->with(["error" => "Attention, il s'agit d'enrégistrement d'un doublon !"]);
		}
    ...
    public function postDepartTeleConseiller(Request $request){
		...
		$tele = DB::table('teleconseillers')
			->where('teleconseillers.id', '=', $presence->teleconseiller_id)
			->first();
        ...
        if(Auth::check()){
			...
			$presence_heure = DB::table('presences')
			->latest('presences.heureDepart')
			->where('presences.user_id', '=', $presence->user_id)
			->first();
			
			$heure = $presence_heure->heureDepart;
			...
			$poste = $user->username;
			  ...
			  $e_mail = $tele->email; //prend l'email de la table
              $objet = "Alerte déconnexion eConvivial";
			  $contenu = "Cher(ère),
				Vous vous êtes déconnecté(e)s à votre compte Téléconseiller à 
					ce jour " .$heure. " au poste " .$poste ;
