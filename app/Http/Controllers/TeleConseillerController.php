...
class TeleConseillerController extends Controller
{
	public function indexT(){
        $teles = Teleconseiller::where("etat", 1)->orderBy("id", "desc")->get();
        //$teles = Teleconseiller::orderBy("id", "desc")->get();
        return view("Admin.Teleconseiller.enregistrementTeleconseiller")
                ->with(["teles" => $teles])
                ->with(["page" => "teleconseiller"]);
    }
	
	public function saveTeleConseiller(Request $request){
        
        
        $teleconseiller = new Teleconseiller();
		
        $teleconseiller->nom = $request["nom"];
        $teleconseiller->prenom = $request["prenom"];
        $teleconseiller->telephone = $request["telephone"];
        $teleconseiller->email = $request["email"];
        $teleconseiller->etat = 1;
		//dd($teleconseiller);
        $teleconseiller->save();

        return redirect()->back()->with(["message" => "Téléconseiller(ère) enrégistré(e) avec succès"]);
    }
	
	public function postEditTeleConseiller(Request $request){
        $teleconseiller = Teleconseiller::where("id", $request["id"])->first();
        ...
        return redirect()->back()->with(["message" => "Téléconseiller(ère) modifié(e) avec succès"]);
    }

    public function postDeleteTeleConseiller(Request $request){
         $teleconseiller = Teleconseiller::where("id", $request["id"])->first();
        if($teleconseiller == null){
            return redirect()->back()->with(["erreur" => "Une erreur s'est produite, impossible de modifier."]);
        } 
		
        $teleconseiller->etat = 0;
		$teleconseiller->save();
    ...
    public function getAdminSuivreConseiller()
    {
        ...
        $teleconseiller->telephone;
        $teleconseiller->email ;
        $teleconseiller->etat = 1;
		//dd($teleconseiller);
        $teleconseiller->save();
