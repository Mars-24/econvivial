...
class LoginController extends Controller
{
            ...
            if ($user != null) {
                ...
                if ($user->numero != null) {
                    ...
                    if (Auth::attempt(["numero" => $user->numero, "password" => $request["password"]])) {
                        ...
                        } else if (Auth::user()->typeuser->libelle == "Vaccination") {
                            return redirect()->route("nouveau-beneficiaire-vaccin");
                        } else if (Auth::user()->typeuser->libelle == "Planning FA") {
                            return redirect()->route("nouveau-beneficiaire-planning-familial");
                        }else if (Auth::user()->typeuser->libelle == "Admin FS") {
                            return redirect()->route("admin-formation-sanitaire-grossesse-en-cours");
                        }else if (Auth::user()->typeuser->libelle == "super_admin_fs") {
                            return redirect()->route("super-admin-formation-sanitaire-get-formation-sanitaire");
                        }
    ...
    public function postRecupererPassword(Request $request)
    {
        ...
        if (filter_var($request["numero"], FILTER_VALIDATE_INT)) {
            ...
            //$nombre = 90306374;
            //$user = User::where("numero","+228 ".$nombre)->first();
